...
Mysql_Client::Mysql_Client(const string &ips, const int port, const string &name,
...
bool Mysql_Client::ConnectToMysql()
{
    ...
    if (mysql_real_connect(&mysql_con,
                           ...
                           db_passwd.c_str(),
                           db_name.c_str(),
                           sql_port, NULL, 0) == nullptr)
    {
        return false;
    }
    return true;
}
bool Mysql_Client::Mysql_TelRegister(const string &tel, const string &name, const string &passwd)
{
    // insert into user_info values(0,'13700000001','小白','123456','1',CurDate());
...
bool Mysql_Client::Mysql_Schedule(const string &tel, const string &ticket_id)
{
    ...
    string get_num = string("select tk_max, tk_num from ticket_table where tk_id=") + ticket_id;
    ...
    string set_num = string("update ticket_table set tk_num=") + to_string(curnum) + string(" where tk_id = ") + ticket_id;
    ...
    string yd_sql = string("insert into yd_table values(0,") + tel + string(",") + ticket_id + string(",now(),1)");
...
bool Mysql_Client::MySql_Get_TelRegInfo(const string &tel, Json::Value &resval)
{
    // select yd_id ,tk_name,tk_time from yd_table, ticket_table where yd_table.tk_id = ticket_table.tk_id and yd_table.tel=13700000001;
    string sql = string("select yd_id ,tk_name,tk_time from yd_table, ticket_table where yd_table.tk_id = ticket_table.tk_id and yd_table.tel=") + tel;
    if (mysql_query(&mysql_con, sql.c_str()) != 0)
    {
        return false;
    }
...
bool Mysql_Client::Mysql_CancelTelRegister(const string &ticket_id)
{
    ...
    string get_tkid_sql = string("select tk_id from yd_table where yd_id=") + ticket_id;
    ...
    string get_tknum = string("select tk_num from ticket_table where tk_id=") + tk_id;
    ...
    string set_num = string("update ticket_table set tk_num=") + to_string(tk_num) + string(" where tk_id=") + tk_id;
    if (mysql_query(&mysql_con, set_num.c_str()) != 0)
    {
        mysql_query(&mysql_con, "rollback");
        return false;
